...
public class DataHolder {
    private static DataHolder instance; // Статичка инстанца

    private final StockRepository stockRepository;
    private final StockDataRepository stockDataRepository;
    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ofPattern("M/d/yyyy");

    private DataHolder(StockRepository stockRepository, StockDataRepository stockDataRepository) {
        this.stockRepository = stockRepository;
        this.stockDataRepository = stockDataRepository;
    }

    // Јавен метод за добивање на единствената инстанца
    public static DataHolder getInstance(StockRepository stockRepository, StockDataRepository stockDataRepository) {
        if (instance == null) {
            synchronized (DataHolder.class) { // Thread-safe иницијализација
                if (instance == null) {
                    instance = new DataHolder(stockRepository, stockDataRepository);
                }
            }
        }
        return instance;
    ...
    public void init() {
        try {
            ...
            while ((line = reader.readLine()) != null) {
                System.out.println("Reading line: " + line); // Log секоја линија
                if (isHeader) {
                    isHeader = false;
                    continue;
                }
